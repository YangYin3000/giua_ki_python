...
df_gnp['year'] = pd.to_numeric(df_gnp['year'], errors='coerce')
df_gnp[' GNP (Billions of USD)'] = pd.to_numeric(df_gnp[' GNP (Billions of USD)'], errors='coerce')
...
def clean_dataframe(df):
    ...
    for column in df.columns[2:]:
        df_clean[column]=df_clean[column].apply(lambda x: float(x.replace(',', '').replace('(', '-').replace(')', '')) if isinstance(x, str) else x)
    df_clean[' Vốn đăng ký cấp mới (triệu USD) '] = pd.to_numeric(df_clean[' Vốn đăng ký cấp mới (triệu USD) '], errors='coerce')
    df_clean[' Vốn đăng ký điều chỉnh (triệu USD) '] = pd.to_numeric(df_clean[' Vốn đăng ký điều chỉnh (triệu USD) '], errors='coerce')
    df_clean[' Giá trị góp vốn, mua cổ phần (triệu USD) '] = pd.to_numeric(df_clean[' Giá trị góp vốn, mua cổ phần (triệu USD) '], errors='coerce')
